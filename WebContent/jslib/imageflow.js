...
function ImageFlow() {
    ...
    this.defaults =
	{
	    ...
	    onClick: function () {   
	    	...
	    	var url=this.alt	//这是jsp页面传过来的url
	    	var id = this.lang;
    		layer.confirm('您将要对图片如何操作呢？', {
	    	    btn: ['删除','分享'] //按钮
	    	}, function(){ 
	    			$.post(""+url+"/picture/delete.action?id="+id,function(data){
						location.href=""+url+"/uploading/photo.jsp";
					});	    		
	    	}, function(){ 		
	    		$.post(""+url+"/picture/key.action?share=1&id="+id,function(data){
					location.href=""+url+"/uploading/photo.jsp";
				});
	    	});
    	    	
	    },   /* Onclick behaviour 用户点击图片是，图片的跳转*/
	    ...
	    startID: "",              /* Image ID to begin with */
