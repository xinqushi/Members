...
(function () {
    ...
    global.extend({
        setChartDataUrl: function (url, format, silent) {
        	...
        	var ps = global.dataUrlParams.join("&");
			var joinChar;
			if (null != ps && ps.length > 0) {
				joinChar = url.indexOf("?") == -1 ? "?" : "&";
				url = [url, joinChar, ps].join("");
			}
            /**
            ...
            xStore[this.id] = global.ajax.get(decodeURIComponent(url), function (responseText, x) {
                ...
                /* change by Wu,Yang 2011-09-14 防止多次提交ajax
...
(function () {
    ...
    global.extend({
    	...
    	setDataUrlParams: function(name, value) {// add Wu,Yang
        	global.dataUrlParams[global.dataUrlParams.length] = [name, "=", value].join("");
    	},
